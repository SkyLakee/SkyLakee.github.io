...
S.UI = (function () {
  ...
  function getAction(value) {
    value = value && value.split(' ')[0];//判断value是否为null、‘’、0等假值
    var a = value && 
    value[0] === cmd && value.substring(1);
  ...
  function performAction(value) {
    ...
    timedAction(function (index) {
      ...
      action = getAction(current);//去除#号
      value = getValue(current);//获取字符串数组空格后的数字

      switch (action) {
        case 'countdown':
          value = parseInt(value) || 10;//判断value是否为空，不为空就为本身，否则默认为10
          value = value > 0 ? value : 10;//判断value是否大于零，如果大于零就等于value
